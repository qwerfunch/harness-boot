...
## 2026-05-04T03:35:30Z — `--gate <name> <result>` two-arg option parsing rejects valid input
- **Source**: /harness-boot:work [E2E sample-project verification, post v0.14.0]
- **Category**: bug
- **Severity**: annoying
- **What happened**: `node bin/harness work F-001 --gate gate_5 pass --note "smoke OK" --harness-dir <p>` exits with `error: --gate takes two values: <name> <result>`. commander parses only the first arg (`gate_5`) and treats `pass` as a separate positional, even though the option spec is declared `--gate <name> <result>`. Reproducible against a fresh sample project. `--run-gate <name>` (auto-detect path) works fine, so the issue is contained to the manual-record code path that the slash-command Typical Scenario in `commands/work.md` advertises.
- **Suggested fix**: either (a) replace the dual-arg option with two separate flags (`--gate-name X --gate-result pass`) — most ergonomic in commander; or (b) accept a single space-separated string `--gate "gate_5 pass"` and split inside the action; or (c) drop the manual-record CLI form and rely on `--run-gate` (since gate runner auto-detection is the recommended path anyway).
- **Resolution**: F-120 (v0.14.1) switches the option spec from `.option('--gate <name> <result>', ...)` to `.option('--gate <values...>', ...)` (commander variadic). Action-side `Array.isArray(gateArgs) && length === 2` check stays as the cardinality guard, so `--gate gate_5` (one arg) still exits 3 with the same `--gate takes two values` message. Two regression tests in `tests/parity/cli.test.ts` (F-120 happy path + F-120 cardinality guard) pin the contract.

## 2026-05-04T17:15Z — F-034 pre-commit hook rejects merge commits (no active feature)

- **Source**: /harness-boot:work F-123 PR conflict resolution
- **Category**: ergonomics · missing-feature
- **Severity**: annoying
- **What happened**: After F-122 merged into develop, F-123 PR (`feat/v0.14.3-claude-md-refresh`) developed a conflict on spec.yaml + state.yaml + mirror. Resolution required `git merge origin/develop` and re-staging the merged files. F-034 pre-commit hook then rejected the merge commit because `state.yaml.session.active_feature_id == null` (F-123 had already been completed earlier in the cycle, and merge-conflict-resolution is not a new feature lifecycle). The hook's whitelist (`.harness/state.yaml`, `.harness/_workspace/*`, `CHANGELOG.md`) does not cover the F-N spec entries that get re-staged during a merge.
- **Suggested fix**: F-034 should detect merge-in-progress (`.git/MERGE_HEAD` exists) and short-circuit to `exit 0` for that single commit. The Iron Law audit trail is already preserved in the pre-merge feature_done event; the merge commit is a downstream operation, not a new state transition. Documented as a candidate Tier 2 hardening.
- **Resolution**: Closed in **F-127** (PR pending) — `hooks/pre-commit-phase2.sh` gains a new Branch 3 that exits 0 silently when any of `.git/MERGE_HEAD`, `.git/CHERRY_PICK_HEAD`, `.git/REVERT_HEAD`, `.git/rebase-merge/`, or `.git/rebase-apply/` exists. The discipline still applies to user-driven feature commits; it now correctly steps aside for git's own conflict-resolution finalizers.
