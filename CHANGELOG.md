...
**Adaptive drive + structural archive separation + plugin-level writing rules.** Twelve features (F-129 → F-140) shipped as one minor bump because the batch crosses three substantial axes: (1) Iron Law gains paired `perf_regression` / `perf_resolved` evidence kinds so a perf-fix cycle that worsened the numbers can no longer ship silently. (2) `complete()` now relocates done feature bodies from `spec.yaml` into a sibling `spec.archive.yaml`, and `harness sync` migrates pre-existing bodies in one pass — `spec.yaml` stays slim by construction. (3) `harness drive` becomes adaptive: deterministic replan after each completion, periodic real-test injection, and transient-retry on flaky e2e.

Existing-user migration: `/plugin update` plus the next `harness sync` produces one large diff (bodies move from `spec.yaml` to `spec.archive.yaml`, content unchanged). Copy the new "Response writing rules" section from `docs/templates/starter/CLAUDE.md.template` into the existing `CLAUDE.md` once for the new conventions; agents auto-update.

### Added

- **F-129** — `perf_regression` / `perf_resolved` evidence kinds plus a `complete()` guard that refuses the transition while the latest perf marker is unresolved. Closes the Iron Law gap where a perf-fix cycle could ship even when the just-measured numbers regressed. `--hotfix-reason` bypasses with the existing audit trail.
- **F-131** — new read-only `harness export-spec [--active-only] [--output <path>]` CLI. Compacts done feature bodies for context-conscious LLM imports (75 % size reduction on the canonical spec). Schema-preserving and CQS — never mutates source files.
- **F-132** — body-to-archive separation in `complete()`. New `src/spec/archive.ts` moves `description` + `acceptance_criteria` to `.harness/spec.archive.yaml`; meta keys (id, name, type, area, digest) stay live. Idempotent and best-effort — failures never regress the lifecycle.
- **F-135** — plugin-level **response writing rules** (`docs/communication-rules.md`) promoted from maintainer memory. Two families: answer-first / scannable, and native tone for any user language (Korean / Japanese / Chinese / Spanish / etc.). Wired into `CLAUDE.md.template` and every `agents/*.md`.
- **F-137** — `harness sync` bulk-migrates existing done bodies into the archive in one pass. Idempotent, skips on a dirty working tree, opt-out via `harness.yaml archive.auto_migrate: false` or `harness sync --no-archive-migrate`.
- **F-138** — drive replan hook. After each feature done, drive deterministically defers blocked / superseded ids and drops a manifest when the retro mentions a pivot keyword. Cycle A1 of the drive-adaptivity plan.
- **F-139** — drive mid-cycle real test. Every N done features (default 3) drive runs a user-defined `harness.yaml drive.real_test.command` to catch cross-feature regressions per-feature `gate_5` cannot see. Unconfigured projects skip silently.
- **F-140** — drive transient retry. A single `real_test_failed` no longer halts immediately — first fail schedules an auto-retry, only the second consecutive fail yields. A pass resets the counter. Configurable cap (default 1).

### Changed

- **F-130** — `docs/schemas/spec.schema.json` caps `features[].name` at 100 characters (both the items shape and the Walking Skeleton prefixItems shape) so dashboards and kickoff templates stay readable. Two pre-existing entries shortened to fit.
- **F-133** + **F-134** — pre-commit hook (`F-034`) whitelist extended to recognise the F-132 lifecycle artifacts (`.harness/spec.yaml`, `.harness/spec.archive.yaml`, plus `docs/samples/...` mirrors) so post-complete record commits pass when active_feature_id is null.
- **F-136** — communication rules tightened from F-135. Single English template (Korean side-by-side duplication removed), plus two new Family 1 rules: progress-ID surface discipline (don't list handled F-N IDs in routine responses) and brevity for write surfaces (CHANGELOG / commit / PR follow the same answer-first standard).
...
- All twelve features completed via the harness CLI cycle (gate_0/1/2 auto + gate_5 manual + declared evidence + commit + complete) on individual `feat/v0.14.4-*` branches; PRs #58 / #59 / #60 / #61 / #62 / #63 / #64 / #65 merged into develop.
