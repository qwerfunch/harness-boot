...
- **F-140** — drive transient retry. A single `real_test_failed` no longer halts drive immediately. The first failure schedules an automatic re-run on the next iteration; only the second consecutive failure yields to the user. A successful real test resets the counter. Configurable: `harness.yaml drive.real_test.transient_retry: false` disables; `drive.real_test.max_transient_retries: N` raises the cap (default 1). Closes the "auto-progress unless big decision" loop intent — flaky e2e no longer demands constant babysitting.
- **F-139** — drive mid-cycle real test. After every N feature completions inside a goal (default N=3), drive runs the user-defined `harness.yaml drive.real_test.command` to confirm the accumulating system still works (per-feature `gate_5` only covers unit-level smoke). Pass writes `real_test_passed`; fail writes `real_test_failed` (with stderr tail) and halts the loop with `real_test_failed`. Unconfigured projects skip silently — no behaviour change.
...
- **F-132** — body-to-archive separation in `complete()` lifecycle. The first of the *root-cause* cycles after the `logcat-on` Lite compaction (F-131) reframed spec.yaml's bloat as a structural problem: the file is forced to carry both the currently living definition and the full history because BR-002 says spec is SSoT. Compression rebloats over time; this cycle moves the body out of the live file entirely. New internal module `src/spec/archive.ts` exposes `moveToArchive(harnessDir, fid)` which reads the live spec.yaml, extracts the named feature's `description` and `acceptance_criteria`, appends them to `<harnessDir>/spec.archive.yaml` under the same id, and rewrites the live spec with those two keys removed (every other meta key — id, name, type, area, digest, ui_surface, performance_budget, supersedes — is preserved verbatim). `src/work.ts complete()` calls it after the successful `state.setStatus` save; failure is best-effort with a stderr warn so the lifecycle never regresses. Idempotent — second move on the same id is a no-op for the live file and an overwrite-with-same-bytes for the archive. **No new CLI subcommand**; the move happens automatically inside the existing `harness work F-N --complete` flow. **Scope discipline**: only F-132 itself moves to archive in this cycle (self-bootstrap proof); the 130 existing done entries stay full-bodied in spec.yaml — a separate bulk-migration follow-up cycle will sweep them. 11 vitest cases for `archive.ts` plus 4 cases for the `complete()` hook (lifecycle pass-through on archive-write failure included).
