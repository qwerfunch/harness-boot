...
- **Drift × Iron Law gating** (F-048, F-169) — `complete()` calls `src/check.ts` first. `severity="error"` findings on the wire-integrity drift kinds reject the transition. <!-- harness:fact key=blocking_drift_kinds_count value=5 source=src/work.ts:BLOCKING_DRIFT_KINDS --> Five kinds (`Code · Stale · AnchorIntegration · Coverage · ContentDrift`) <!-- /harness:fact --> currently block; `--hotfix-reason` bypasses with audit trail.
...
- **Drift detection** — <!-- harness:fact key=drift_kinds_count value=15 source=src/check.ts:DriftKind --> 15 kinds <!-- /harness:fact -->: Generated · Derived · Spec · Include · Evidence · Code · Anchor · Adr · Stale · AnchorIntegration · Doc · Protocol · Coverage (description-vs-fingerprint substantive coverage) · AcceptanceTrace (F-168, opt-in AC ↔ test mapping) · ContentDrift (F-169, CLAUDE.md sigil ↔ code SSoT validation). Two-layer supersession metadata (`features[].supersedes` / `superseded_by`) and the archive flow.
...
- **`legacy/` is fully untracked** (F-125). Pre-cutover Python source remains as a local-only archive on the maintainer's machine for reference; do not re-add it to git.
