...
- **Drift detection** — <!-- harness:fact key=drift_kinds_count value=14 source=src/check.ts:runCheck --> 14 kinds <!-- /harness:fact -->: Generated · Derived · Spec · Include · Evidence · Code · Anchor · Adr · Stale · AnchorIntegration · Doc · Protocol · Coverage (description-vs-fingerprint substantive coverage) · AcceptanceTrace (F-168, opt-in AC ↔ test mapping). Two-layer supersession metadata (`features[].supersedes` / `superseded_by`) and the archive flow.
