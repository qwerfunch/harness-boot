...
Claude> F-3 (로그인) 시작.
        완료 조건: gate_5 (스모크 실행) PASS + 근거 1 건 이상.
        다음: 검증 실행.

사용자> 돌려봐

Claude> gate_0  (테스트)             PASS    — 단위 19 개
        gate_5  (스모크 실행)        PASS    — 근거 자동 기록
        gate_1/2/3  (타입/스타일/커버리지)  SKIPPED — 도구 미감지
        Iron Law 충족 (gate_5 + 근거). F-3 끝낼까요? [Y/n]
