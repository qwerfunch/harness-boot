...
> Claude Code 플러그인. 자연어 아이디어를 **스펙으로 굳히고**, 전문가 에이전트 팀이 **역할별로 협업해** 실제 돌아가는 코드까지 이끌어낸다.
...
- **무엇**: AI 개발 루프에 규율을 걸어주는 Claude Code 플러그인.
- **누구**: Claude Code 로 혼자 또는 소규모 팀에서 제품을 만드는 개발자.
- **어떻게**: `spec.yaml` 한 파일에 기획·계약·결정을 적고, 플러그인이 파생 뷰·전문가 에이전트·협업 루틴을 얹어준다.
...
| v0.7 (계획) | 루틴 자동 연동 · 에이전트별 fixture 확장 · 평가 하네스 · [알려진 제약](#알려진-제약-v07-에서-해소) 실 구현 |
...
- **외부 도그푸드**: `suika-web` (수박게임 웹, v0.5 워크플로 역할극 + v0.6.1 협업 루틴 스크립트 실증)

---

## 알려진 제약 (v0.7 에서 해소)

v0.6.1 시점에 **설계는 있지만 구현이 미완**인 항목들:

- 협업 루틴 자동 연동 — 현재는 orchestrator 가 수동 호출 (v0.7 에서 `scripts/work.py` 가 subprocess 로 자동 호출).
- ADR 의 `supersedes` 자동 전이 — 렌더만 됨, 이전 ADR 의 `status=superseded` 로의 자동 갱신 미구현.
- `skipped_agents[]` 이력 기록 — 스키마만 있고 `state.py` 에서 읽기/쓰기 없음.
- `performance_budget` 게이트 연동 — 스키마만 있고 `gate_runner.py` 에 소비자 없음.
- Design 계층 에이전트의 플랫폼 맥락 접근 — 모션/오디오 결정이 플랫폼 의존이나 현재 Tier 1 만.

자세한 내용은 [`CHANGELOG.md`](CHANGELOG.md) 의 `[0.6.1]` 섹션 "Known" 항목.
...
**Q. 작은 프로젝트엔 과하지 않나요?**
네, 과합니다. 피처 3 개 미만이면 `/harness:init` + `/harness:spec` 만으로 충분합니다. 전문가 에이전트와 루틴은 소환 안 해도 됩니다. 규모가 커지면서 필요한 순간에 붙이는 계층 구조입니다.
...
v0.6.1 은 **자체 검증 (535 tests · self_check)** 은 통과했습니다. 다만 **실제 Claude Code 세션에서 전체 에이전트 연쇄 호출 실증**은 v0.7 에서 보강됩니다. 규율 도구로 당장 도입 가능하되, 16 에이전트 전체를 한 피처에 소환하는 대규모 워크플로는 v0.7+ 를 권장합니다.
