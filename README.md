...
사용자가 **직접 편집하는 파일은 `spec.yaml` 하나** 입니다. 나머지는 전부 자동 파생. 파생이 원천과 어긋나면 알려줍니다 (단, 멋대로 덮어쓰지 않습니다).

---

## 이런 분에게 유용합니다

- Claude Code 로 **혼자 또는 소규모** 제품을 만들면서, AI 가 스스로 "다 됐다" 선언해 버리는 걸 통제하고 싶은 분
- 한 줄 아이디어를 **일관된 방식** 으로 배포 가능한 코드까지 밀고 싶은 분
- 여러 기능이 쌓이면서 **결정 이력 · 파일 드리프트 · 감사 로그** 가 필요해지는 프로젝트
- 팀 규모를 늘리지 않고 **frontend · backend · 보안 · 접근성 · 품질** 같은 역할을 AI 에이전트로 분산하고 싶은 분
...
| **단일 원천** | 사용자 편집 대상은 `spec.yaml` 하나. 나머지는 전부 파생 |
