...
- **누구**: **적은 인원으로 큰 시스템을 만들려는 개발자·팀**. 전통적으로 수십 명이 했던 규모를 AI 에이전트 레버리지로 1~3 인이 수행하는 방향.
...
**Q. 작은 프로젝트엔 과하지 않나요?**
이 도구의 본래 의도는 **작은 팀이 큰 시스템을 만드는 것**이라 피처 3 개 미만의 토이 프로젝트에는 오버킬일 수 있습니다. 단 **10~15 피처 이상의 실제 제품** 부터는 규율의 가치가 비용을 넘어섭니다. 규모가 커질수록 이 도구가 드러내는 레버리지 (전문가 에이전트 · 협업 루틴 · 감사 이력) 가 더 중요해지므로, 작은 프로젝트에 과하게 쓰지 말되 **큰 것을 만들 때 필수** 라 생각하세요.
