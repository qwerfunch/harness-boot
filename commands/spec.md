...
3. 각 답변 후 `scripts/core/canonical_hash.py` 로 해시 기록하며 진척 확인.
...
2. `skills/spec-conversion` SKILL.md v0.5 invocation:
   Stage 1 정찰 — Bash python3 scripts/spec/mode_b_extract.py <plan.md>
     → BM25 통계 + 도메인 힌트
...
3. Schema 검증: `python3 scripts/spec/validate.py <new-spec>`
   실패 시 §Stage 2 로 복귀
```
...
4. `scripts/spec/diff.py <old> <new> --yaml` 로 diff 렌더
```
...
3. 개선 제안 (3 축 우선):
   ...
   - 간결성 (중복 표현 병합)
4. `scripts/spec/diff.py` diff 렌더
```
...
```
1. 대상 범위 파싱:
   - 전체: scripts/spec/explain.py <spec.yaml>
   - 피처: scripts/spec/explain.py <spec.yaml> --feature F-003
   - 엔티티: scripts/spec/explain.py <spec.yaml> --entity User
...
- **Mode 분류기** (결정론): `scripts/spec/mode_classifier.py --args "..." --spec-exists true|false`
  → `{"mode": "A|B|R|E", "rationale": "...", "subtype": "..."}` JSON 반환
- **Mode E (read-only)**: `scripts/spec/explain.py <spec.yaml> [--feature F-X | --entity Y] [--json]`
  → overview / feature / entity 요약 · `mtime` 불변 (CQS 테스트 검증)
- **Mode A/R diff**: `scripts/spec/diff.py <old> <new> [--yaml | --stat | --json]` 또는 `spec_diff.py <spec> --git-head`
...
- `scripts/spec/mode_b_extract.py` + `scripts/spec/mode_b/{axes,stopwords,roundtrip}.py` — 통계 추출 구현 (v0.7.6 재배치).
- `scripts/spec/upgrade_to_2_3_8.py` — Mode R 의 스키마 마이그레이션 보조.
