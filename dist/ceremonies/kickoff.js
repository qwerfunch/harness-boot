...
export function hasAudioFlag(feature) {
 ...
 */
export function detectShapes(feature, spec = null) {
    if (!isPlainObject(feature)) {
        return [];
    }
 ...
 */
export function agentsForShapes(shapes, hasAudio = false) {
    const out = [];
    const seen = new Set();
 ...
 */
export function parallelGroupsForShapes(shapes, hasAudio = false) {
    const groups = [];
    const seen = new Set();
    for (const shape of shapes) {
 ...
 */
export function renderStyleBlock(harnessDir, feature) {
    ...
    lines.push('');
    lines.push('> 어둠이 걷힌 영역. 아래 chapter 가 implementer / software-engineer / frontend-engineer 의 기본 컨텍스트.');
    lines.push('');
...
function template(featureId, agents, timestamp, mode, styleBlock) {
 ...
 * (unless `force=true`).
 */
export function generateKickoff(harnessDir, featureId, shapes, options = {}) {
    ...
    if (agents.length === 0) {
        throw new Error(`no agents matched for shapes=${JSON.stringify([...shapes])}; check ROUTING_SHAPES`);
    }
