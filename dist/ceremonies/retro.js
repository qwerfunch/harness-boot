 ...
 */
export function analyze(events, featureId) {
...
function template(featureId, analysis, timestamp, mode) {
    const isPrototype = mode === 'prototype';
    const fgf = analysis.first_gate_fail;
    const fgfLine = fgf !== null
        ? `- ${fgf['gate'] ?? '?'} failed at ${fgf['ts'] ?? '?'}` +
            `  (reason: ${fgf['note'] || fgf['reason'] || '?'})`
        : '- 없음 (전 gate 최초에 pass)';
    ...
    if (analysis.archived) {
        ...
        if (typeof sb === 'string' && sb.length > 0) {
            ...
            lines.push(`- 이 피처는 archived 됨 (${ts}) — superseded_by 미지정 (deprecation only · 대체 피처 없음)`);
        }
        lines.push(`- 사유: ${reason}`);
        lines.push('');
        lines.push('<!-- F-028: feature_archived event 의 superseded_by/reason 자동 채움. 수동 추가 컨텍스트는 아래에 자유 기술. -->');
        lines.push('');
    }
    // First Gate to Fail
    ...
    lines.push(`- Design Review opened: ${analysis.design_review_opened ? '✅' : '❌ (해당 피처에 미실행)'}`);
    lines.push(`- Questions opened: ${analysis.questions_opened} · answered: ${analysis.questions_answered}`);
    lines.push('');
    if (isPrototype) {
    ...
    lines.push('');
    lines.push('<!-- orchestrator invokes @harness:reviewer to produce draft prose. ' +
    ...
    lines.push('');
    lines.push('<!-- orchestrator invokes @harness:tech-writer to polish the Reviewer Reflection. ' +
 ...
 */
export function generateRetro(harnessDir, featureId, options = {}) {
