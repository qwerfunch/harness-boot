 ...
 */
export function _strategyPathBin(pluginName) {
 ...
 */
export function _strategyRegistry(pluginName) {
 ...
 */
export function _strategyMarketplaceSource(pluginName) {
 ...
 */
export function resolve(pluginName = 'harness') {
    const attempts = [];
    ...
    throw new PluginRootError(`플러그인 '${pluginName}' 루트 해석 실패 — attempts: ${attempts.join(', ')}`);
}
/** Predicate convenience — returns true when {@link resolve} would succeed. */
export function isResolvable(pluginName = 'harness') {
    try {
        resolve(pluginName);
        return true;
    }
...
//# sourceMappingURL=pluginRoot.js.map