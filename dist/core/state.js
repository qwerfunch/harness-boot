...
export const GATE_RESULTS = ['pass', 'fail', 'skipped'];
/**
 ...
 */
export const AUTOMATIC_EVIDENCE_KINDS = new Set(['gate_run', 'gate_auto_run']);
/**
...
export class State {
    ...
    setStatus(fid, status) {
        if (!FEATURE_STATUSES.includes(status)) {
            throw new Error(`invalid status '${status}' (expected one of ${FEATURE_STATUSES.join(', ')})`);
        }
    ...
    removeFeature(fid) {
        ...
        this.data.features = this.data.features.filter((f) => !(isPlainObject(f) && f.id === fid));
 ...
 * list.
 */
export function isDeclaredEvidence(ev) {
    if (!isPlainObject(ev)) {
        return false;
    }
    const kind = ev.kind;
 ...
 */
export function countDeclaredEvidence(feature, options = {}) {
