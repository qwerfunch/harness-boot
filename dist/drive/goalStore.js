 ...
 */
export function nextGoalId(existing) {
    let max = 0;
 ...
 */
export function readGoals(specPath) {
    ...
    for (const g of goals) {
        ...
        const featureIds = Array.isArray(g.feature_ids)
            ? g.feature_ids.filter((x) => typeof x === 'string')
            : [];
        out.push({
            ...g,
            id: g.id,
 ...
 */
export function createGoal(input, existingIds) {
 ...
 */
export function appendGoal(specPath, goal) {
    const raw = readFileSync(specPath, 'utf-8');
    const doc = yamlParse(raw);
    if (!isPlainObject(doc)) {
 ...
 */
export function archiveGoal(specPath, gid, reason, now = new Date()) {
...
//# sourceMappingURL=goalStore.js.map