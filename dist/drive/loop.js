    ...
    if (mapped.kind === 'run_gate' && executed.work !== undefined) {
        ...
        if (stagnated) {
            ...
            return {
                ...
                executor: executed,
                feature_id: mapped.feature_id,
                halt: emitHalt(harnessDir, 'gate_no_progress', `${mapped.gate} on ${mapped.feature_id} produced '${result}' ${window.length} times in a row with no detectable progress — yielding.`, {
                    goal_id: ck.goal_id,
                    feature_id: mapped.feature_id,
                    gate: mapped.gate,
                    iteration: ck.execute.iteration,
