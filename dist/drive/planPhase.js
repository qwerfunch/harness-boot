...
import { createGoal, nextGoalId, readGoals, } from './goalStore.js';
import { defaultCheckpoint, goalArtifactDir, loadCheckpoint, saveCheckpoint, } from './checkpoint.js';
 ...
 */
export function briefPathFor(harnessDir, goalId) {
...
export function planPathFor(harnessDir, goalId) {
 ...
 */
export function startPhaseA(input) {
    ...
    const halt = emitHalt(harnessDir, 'plan_phase_approval', `researcher must compose ${ck.plan.brief_path} for goal "${goalSpec.title}". ` +
        'After approval, run `harness drive --resume`.', { goal_id: goalId, now });
 ...
 */
export function advancePhaseA(harnessDir, approvals = {}, now = new Date()) {
    const ck = loadCheckpoint(harnessDir);
    if (ck === null) {
        return {
            kind: 'halt',
            halt: emitHalt(harnessDir, 'manual', 'no drive checkpoint found — run `harness drive "<goal>"` first.', { now }),
        };
    }
    if (ck.phase !== 'planning') {
        return {
            kind: 'halt',
            halt: emitHalt(harnessDir, 'manual', `phase ${ck.phase} is past Phase A — use --resume for the execute loop.`, { goal_id: ck.goal_id, now }),
        };
    }
    ...
    if (!existsSync(ck.plan.brief_path)) {
        return {
            ...
            halt: emitHalt(harnessDir, 'plan_phase_approval', `researcher's brief is missing at ${ck.plan.brief_path}. Have the researcher agent write it, then resume.`, { goal_id: ck.goal_id, now }),
        };
    }
    ...
    if (!ck.plan.brief_approved) {
        if (!approvals.autoApproveBrief && !approvals.autoApproveAll) {
            ...
            return {
                kind: 'halt',
                briefPath: ck.plan.brief_path,
                halt: emitHalt(harnessDir, 'plan_phase_approval', `brief.md is present at ${ck.plan.brief_path}. Review, then resume to dispatch product-planner.`, { goal_id: ck.goal_id, now }),
            };
        }
    ...
    if (!existsSync(ck.plan.plan_path)) {
        return {
            ...
            halt: emitHalt(harnessDir, 'plan_phase_approval', `product-planner's plan is missing at ${ck.plan.plan_path}. Have the planner agent write it, then resume.`, { goal_id: ck.goal_id, now }),
        };
    }
    ...
    if (!ck.plan.plan_approved) {
        if (!approvals.autoApproveAll) {
            ...
            return {
                ...
                briefPath: ck.plan.brief_path,
                planPath: ck.plan.plan_path,
                halt: emitHalt(harnessDir, 'plan_phase_approval', `plan.md is present at ${ck.plan.plan_path}. Review, then resume to dispatch feature-author for the scaffolding.`, { goal_id: ck.goal_id, now }),
            };
        }
        ck.plan.plan_approved = true;
        saveCheckpoint(harnessDir, ck, now);
    ...
    if (scaffolded.length === 0) {
        return {
            ...
            briefPath: ck.plan.brief_path,
            planPath: ck.plan.plan_path,
            halt: emitHalt(harnessDir, 'plan_phase_approval', `feature-author has not scaffolded any features for ${ck.goal_id} yet. ` +
                'Author the features into both spec.yaml mirrors with feature-author, then resume.', { goal_id: ck.goal_id, now }),
        };
    }
 ...
 */
export function readGoalContext(harnessDir, goalId) {
...
//# sourceMappingURL=planPhase.js.map