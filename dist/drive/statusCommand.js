...
import { renderProgress, renderProgressJson } from './progressRenderer.js';
...
function readSpecFeatures(harnessDir) {
    ...
    return doc.features.filter(isPlainObject);
}
/**
...
function enrichFeatures(runtimeFeatures, specFeatures) {
    ...
    return runtimeFeatures.map((f) => {
        ...
        return name === undefined ? f : { ...f, name };
    });
}
/**
 ...
 *   {@link runDriveStatus}.
 */
export function composeStatusText(goals, state, specFeatures, options) {
 ...
 */
export function composeStatusJson(goals, state, specFeatures, options) {
    const selected = selectGoals(goals, state, options);
    const out = [];
    for (const goalSpec of selected) {
        const goalRuntime = state.getGoal(goalSpec.id);
 ...
 */
export async function runDriveStatus(options) {
