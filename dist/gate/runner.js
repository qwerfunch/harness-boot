...
export function detectGate0Command(projectRoot) {
...
function hasTestFiles(dir) {
    try {
        return readdirSync(dir).some((name) => name.startsWith('test_') && name.endsWith('.py') && isFile(join(dir, name)));
    }
    catch {
        return false;
    }
...
export function detectGate1Command(projectRoot) {
    const pyproject = join(projectRoot, 'pyproject.toml');
    if (isFile(pyproject)) {
        if (which('mypy') !== null) {
...
export function detectGate2Command(projectRoot) {
    const pyproject = join(projectRoot, 'pyproject.toml');
    if (isFile(pyproject)) {
        if (which('ruff') !== null) {
            return ['ruff', 'check', '.'];
...
export function detectGate3Command(projectRoot) {
    const pyproject = join(projectRoot, 'pyproject.toml');
    if (isFile(pyproject)) {
        const cmd = pytestCommand();
        if (cmd !== null) {
...
export function detectGate4Command(projectRoot) {
...
const PW_CONFIG_NAMES = [
    'playwright.config.ts',
    'playwright.config.js',
    'playwright.config.mjs',
    'playwright.config.cjs',
];
const CY_CONFIG_NAMES = [
    'cypress.config.ts',
    'cypress.config.js',
    'cypress.config.mjs',
    'cypress.config.cjs',
...
export function detectGate5Command(projectRoot) {
...
export function detectGatePerfCommand(_projectRoot) {
    return null;
}
...
function execute(gate, cmd, projectRoot, timeoutSec) {
    ...
    const elapsed = Number((process.hrtime.bigint() - start) / 1000000n) / 1000;
...
/** gate_0 (tests). */
export function runGate0(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_0', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate0Command);
    if (cmd === null) {
        return skippedResult('gate_0', 'no test command detected (pyproject.toml · tests/ · package.json · Makefile 모두 부재)');
    }
    return execute('gate_0', cmd, projectRoot, options.timeoutSec ?? DEFAULT_TIMEOUT_SEC);
}
/** gate_1 (type check). */
export function runGate1(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_1', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate1Command);
    if (cmd === null) {
        return skippedResult('gate_1', 'no type checker detected (pyproject.toml · tsconfig.json · Cargo.toml · go.mod 모두 부재 또는 tool 미설치)');
    }
    return execute('gate_1', cmd, projectRoot, options.timeoutSec ?? DEFAULT_TIMEOUT_SEC);
}
/** gate_2 (lint). */
export function runGate2(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_2', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate2Command);
    if (cmd === null) {
        return skippedResult('gate_2', 'no linter detected (pyproject/ruff · package.json/eslint · Cargo/clippy · go.mod/golangci-lint 모두 부재)');
    }
    return execute('gate_2', cmd, projectRoot, options.timeoutSec ?? DEFAULT_TIMEOUT_SEC);
}
/** gate_3 (coverage). */
export function runGate3(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_3', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate3Command);
    if (cmd === null) {
        return skippedResult('gate_3', 'no coverage tool detected (pytest-cov · nyc · scripts.coverage · tarpaulin · go -cover 모두 부재)');
    }
    return execute('gate_3', cmd, projectRoot, options.timeoutSec ?? 600);
}
/** gate_4 (working tree clean). */
export function runGate4(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_4', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate4Command);
    if (cmd === null) {
        return skippedResult('gate_4', 'not a git repo or git binary 부재 — commit gate 검증 불가');
    }
    return execute('gate_4', cmd, projectRoot, options.timeoutSec ?? 30);
}
/** gate_5 (runtime smoke). */
export function runGate5(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_5', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGate5Command);
    if (cmd === null) {
        return skippedResult('gate_5', 'no runtime smoke detected (scripts/smoke.sh · tests/smoke/ · Makefile smoke · package.json scripts.smoke 모두 부재) — harness.yaml.gate_commands.gate_5 로 설정 필요');
    }
    return execute('gate_5', cmd, projectRoot, options.timeoutSec ?? 600);
}
/** gate_perf (performance budget). */
export function runGatePerf(projectRoot, options = {}) {
    const cmd = resolveCommand('gate_perf', projectRoot, options.overrideCommand ?? null, options.harnessDir ?? null, detectGatePerfCommand);
    if (cmd === null) {
        return skippedResult('gate_perf', 'no perf runner configured — harness.yaml.gate_commands.gate_perf 또는 --override-command 필요');
    }
    return execute('gate_perf', cmd, projectRoot, options.timeoutSec ?? 900);
}
/** Dispatcher — maps a gate name to its runner. */
export function runGate(gate, projectRoot, options = {}) {
    switch (gate) {
        case 'gate_0':
        ...
        default:
            return skippedResult(gate, `${gate} auto-run not yet supported (v0.3.7 shipped gate_0~gate_5)`);
    }
}
