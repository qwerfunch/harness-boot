...
import { collectSignals } from './codebase/signals.js';
/**
 ...
 */
export function autoDetectScenario(projectRoot) {
...
function isCodebaseDirectoryPattern(signals) {
    const pattern = signals.directoryPattern;
    return (pattern === 'src+tests' ||
        pattern === 'colocated' ||
        pattern === 'next-app' ||
        pattern === 'next-pages' ||
        pattern === 'monorepo');
...
function describeCodebaseEvidence(signals) {
    const parts = [];
