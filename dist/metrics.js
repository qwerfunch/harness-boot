...
export function aggregate(events, options = {}) {
    ...
    for (const ev of events) {
        ...
        if (typ === 'sync_failed') {
            ...
            const bucket = report.gate_stats[gate] ??
 ...
 */
export function compute(harnessDir, options = {}) {
    const allEvents = [...readEvents(harnessDir)];
...
export function formatHuman(report) {
    ...
    if (lt.count > 0) {
        lines.push(`Lead time (n=${lt.count}): min ${fmtSec(lt.min)} · median ${fmtSec(lt.median)} · mean ${fmtSec(lt.mean)} · max ${fmtSec(lt.max)}`);
    }
    else {
    ...
    if (Object.keys(report.gate_stats).length > 0) {
        ...
        for (const [gate, b] of Object.entries(report.gate_stats).sort()) {
            ...
            lines.push(`  ${pad(gate, 8)} ${padLeft(String(b.pass), 5)} ${padLeft(String(b.fail), 5)} ${padLeft(String(b.skipped), 5)}   ${rate}`);
        }
    }
    else {
