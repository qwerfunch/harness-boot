 ...
 * reviews.
 */
export const LOCKED_FIELD_NAMES = new Set([
    'id',
    'version',
    'name',
    'type',
 ...
 */
export function findIncludes(obj) {
    const out = [];
    walk(obj, [], null, out);
    return out;
}
...
function readChapter(chaptersDir, rel) {
    ...
    if (relativeFromBase === '' ||
        ...
        throw new IncludeError(`$include 경로가 chapters 디렉터리를 벗어남: ${rel} → ${target}`);
    }
    let isFile;
    try {
 ...
 */
export function expand(spec, chaptersDir, options = {}) {
    ...
    for (const item of includes) {
        if (strictLockedFields &&
            item.parentKey !== null &&
            LOCKED_FIELD_NAMES.has(item.parentKey)) {
            throw new IncludeError(`🔒 필드 \`${item.parentKey}\` 에는 $include 를 사용할 수 없음 ` +
 ...
 */
export function resolveChaptersDir(specPath, explicit = null) {
    if (explicit !== null) {
        return explicit;
    }
...
//# sourceMappingURL=includeExpander.js.map