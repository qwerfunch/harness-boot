...
function suggestionsForIdle(features, spec) {
    const inProgress = features.filter((f) => isPlainObject(f) && f['status'] === 'in_progress' && typeof f['id'] === 'string');
    const planned = features.filter((f) => isPlainObject(f) && f['status'] === 'planned' && typeof f['id'] === 'string');
    const out = [];
 ...
 */
export function suggest(stateData, spec = null, options = {}) {
    if (!isPlainObject(stateData)) {
        return [];
    }
    const session = isPlainObject(stateData['session']) ? stateData['session'] : {};
    ...
    if (coverage !== undefined && coverage !== null && coverage < DEFAULT_COVERAGE_THRESHOLD) {
        ...
        out = [
            {
                label: `Review carry-forward debt — coverage ${pct}% < threshold ${thresholdPct}% ` +
...
//# sourceMappingURL=intentPlanner.js.map