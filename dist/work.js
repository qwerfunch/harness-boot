 ...
 */
const PERF_MARKER_KINDS = new Set(['perf_regression', 'perf_resolved']);
...
function autowireInitialSync(harnessDir) {
    try {
        ...
        if (!result.ok && result.reason !== 'spec.yaml missing') {
            process.stderr.write(`[warn] initial sync auto-wire failed: ${result.reason} — ` +
...
function warnIfGhost(harnessDir, fid) {
    ...
    if (findFeature(spec, fid) === null) {
        process.stderr.write(`warn: ${fid} not defined in spec.yaml — proceeding as ghost feature. ` +
...
function warnIfConcurrent(state, fid) {
    ...
    if (others.length > 0) {
        process.stderr.write(`warn: other feature(s) still in_progress: ${others.join(', ')}. ` +
            'Finish or block before switching, or ignore to work in parallel.\n');
    }
}
/** Activates a feature — planned → in_progress + auto-wires. */
export function activate(harnessDir, fid, options = {}) {
...
export function recordGate(harnessDir, fid, gateName, result, options = {}) {
    const state = State.load(harnessDir);
...
export function addEvidence(harnessDir, fid, kind, summary) {
    const state = State.load(harnessDir);
...
export function block(harnessDir, fid, reason, options = {}) {
 ...
 */
export function complete(harnessDir, fid, options = {}) {
    ...
    if (!hotfixReason) {
        try {
            ...
            const blocking = driftReport.findings.filter((d) => d.severity === 'error' && BLOCKING_DRIFT_KINDS.has(d.kind));
...
export function archive(harnessDir, fid, options = {}) {
    const state = State.load(harnessDir);
    const f = state.ensureFeature(fid);
...
export function current(harnessDir) {
    const state = State.load(harnessDir);
...
export function deactivate(harnessDir) {
    const state = State.load(harnessDir);
    const fid = state.data.session.active_feature_id;
    if (typeof fid !== 'string' || fid.length === 0) {
        return {
...
export function removeFeature(harnessDir, fid) {
    const state = State.load(harnessDir);
    const f = state.getFeature(fid);
    if (f === null) {
        return {
 ...
 */
export function runAndRecordGate(harnessDir, fid, gateName, options = {}) {
    ...
    if (runResult.result === 'pass' && addEvidenceOnPass) {
        ...
        if (gateName === 'gate_perf') {
            ...
            const budgetSummary = formatPerformanceBudget(feature !== null ? feature['performance_budget'] : null);
