...
3. Read `feature-list.json` to identify the current feature's `tdd_focus` functions and `test_strategy`
...
5. Parse structured coverage JSON (not plain text grep) to check function coverage
...
7. If all covered, no tdd_focus defined, or `test_strategy` is not `"tdd"`: **exit 0** (allow)
...
| `test_strategy` | `"tdd"` \| `"state-verification"` \| `"integration"` | No | `"tdd"` | No |
...
|-------|-------------|----------|--------|-------------------|
| `"tdd"` (default) | Pure logic functions (validators, calculators, state machines) | Red → Green → Refactor | Full TDD evidence | tdd_focus 100% line coverage |
...
- Features with pure logic functions → `"tdd"`
...
- Present dependency graph to user for confirmation:
  ```
...
|-----------|-------|-------|-------|
| `tdd-test-writer` | Red / State-Test | Write tests from interfaces only, without reading implementation code | sonnet |
| `tdd-implementer` | Green / Implement | Write only the minimal code needed to pass tests. Apply Comment Rules (Section 7.2) | sonnet |
| `tdd-refactorer` | Refactor | No behavior changes allowed. Verify and supplement Comment Rules compliance | sonnet |
...
| Gate | `tdd` (default) | `state-verification` | `integration` |
|------|-----------------|---------------------|---------------|
| **Gate 0** | Full: test files exist, Red → Green order evidence, happy/boundary/error cases | Relaxed: test files exist, tests pass, state assertions present | Relaxed: integration test file exists, tests pass |
| **Gate 3** | tdd_focus functions: 100% line coverage | Test files exist for module (no per-function coverage) | Overall file coverage >= 60% |
...
If Gate 0 fails: return to Red/Implement phase. This counts toward the 5-iteration convergence limit.
