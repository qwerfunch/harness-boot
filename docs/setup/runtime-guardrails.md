...
# Read test_strategy (default: "lean-tdd")
...
if [[ "$TEST_STRATEGY" == "lean-tdd" ]]; then
  ...
  [[ -z "$FEATURE_ID" ]] && exit 0
  BDD_FILE=$(find "$PROJECT_ROOT" -type f -name "${FEATURE_ID}.bdd.*" 2>/dev/null | head -n 1 || true)
  if [[ -z "$BDD_FILE" ]]; then
  ...
  if [[ "$SCENARIOS" -lt "$EXPECTED" ]]; then
    echo "BLOCKED: BDD file has ${SCENARIOS} scenario(s); acceptance_test requires ${EXPECTED}." >&2
    exit 2
  fi
  exit 0
fi
