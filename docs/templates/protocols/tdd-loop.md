...
| Refactor | `tdd-refactorer` | Green exit OK | No behavior change; suite still PASSES; Comment Rules (setup-guide §7.2) verified |
...
- `tdd-test-writer` MUST NOT read implementation code. Tool list excludes `Edit`; file globs restrict reads to `*.d.ts`, `*.types.*`, test files, and interface headers (see setup-guide.md §5 "File Classification for tdd-test-writer").
