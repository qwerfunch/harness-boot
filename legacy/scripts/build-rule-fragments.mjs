...
function rewriteTargetReferences(text) {
  return text
    ...
    // File Classification is embedded into tdd-test-writer/bdd-writer via Rule 04 at Phase 3 — point there.
    .replace(/`\$\{CLAUDE_PLUGIN_ROOT\}\/docs\/setup\/tdd-isolation\.md#file-classification-for-tdd-test-writer`/g, "this agent's `## File Classification` section")
...
/* ── Rule 12 — Sub-agent Input Sanitization ───────────────────── */
// Shared contract for bdd-writer + tdd-test-writer: both sub-agents rely on
// the same double-barrier (implementer sanitizes before forwarding + sub-agent
// self-checks on receipt) with agent-specific allowed/forbidden lists. Keeping
// this in one place eliminates the 65% structural drift between the two body
// templates and lets the isolation contract evolve from a single source.
extract(
  join(REPO_ROOT, 'commands/start.md'),
  /^#### TDD \/ BDD sub-agent input sanitization.*<!-- anchor: sub-agent-input-sanitization -->/,
  /^#### Per-feature cycle by/,
  join(DEST, '12-subagent-input-sanitization.md'),
);
