...
const CORE_REVIEWERS: ReadonlyArray<string> = [
  'visual-designer',
  'frontend-engineer',
  'a11y-auditor',
];
 ...
 */
export function reviewersFor(hasAudio: boolean): string[] {
...
function template(featureId: string, reviewers: ReadonlyArray<string>, timestamp: string): string {
  const lines: string[] = [];
 ...
 */
export function generateDesignReview(
  harnessDir: string,
  featureId: string,
  options: GenerateDesignReviewOptions = {},
): string {
...
export function designReviewExists(harnessDir: string, featureId: string): boolean {
