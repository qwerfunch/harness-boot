...
export function checkGenerated(
  _harnessDir: string,
  harnessYaml: Record<string, unknown> | null,
): DriftFinding[] {
...
export function checkDerived(
  harnessDir: string,
  harnessYaml: Record<string, unknown>,
): DriftFinding[] {
  const findings: DriftFinding[] = [];
  const generation = isPlainObject(harnessYaml['generation']) ? harnessYaml['generation'] : {};
  const derived = isPlainObject((generation as Record<string, unknown>)['derived_from'])
    ? ((generation as Record<string, unknown>)['derived_from'] as Record<string, unknown>)
    : {};
  ...
  for (const [key, filename] of mapping) {
    const entry = isPlainObject(derived[key]) ? (derived[key] as Record<string, unknown>) : {};
...
export function checkSpec(
  ...
  const generation = isPlainObject(harnessYaml['generation']) ? harnessYaml['generation'] : {};
  const generatedFrom = isPlainObject((generation as Record<string, unknown>)['generated_from'])
    ? ((generation as Record<string, unknown>)['generated_from'] as Record<string, unknown>)
    : {};
...
export function checkIncludes(
  ...
  const findings: DriftFinding[] = [];
  const generation = isPlainObject(harnessYaml['generation']) ? harnessYaml['generation'] : {};
  const recorded = asArray((generation as Record<string, unknown>)['include_sources']).filter(
  ...
  if (isFile(specPath)) {
    try {
      const parsed: unknown = yamlParse(readFileSync(specPath, 'utf-8'));
...
export function checkEvidence(harnessDir: string): DriftFinding[] {
  const findings: DriftFinding[] = [];
  ...
  for (const f of state.data.features) {
    ...
    if (f['status'] === 'done' && (!Array.isArray(f['evidence']) || (f['evidence'] as unknown[]).length === 0)) {
...
export function checkAnchor(spec: Record<string, unknown>): DriftFinding[] {
  const findings: DriftFinding[] = [];
  const features = asArray(spec['features']);
...
function checkFeatureSupersedes(
...
export function checkAdrSupersedes(spec: Record<string, unknown>): DriftFinding[] {
  const findings: DriftFinding[] = [];
...
export function checkProtocol(harnessDir: string): DriftFinding[] {
  const findings: DriftFinding[] = [];
  ...
  for (const name of entries) {
    ...
    try {
      ...
      findings.push({
        kind: 'Protocol',
        path: relative(harnessDir, md),
        message: `frontmatter YAML 파싱 실패: ${(err as Error).message}`,
        severity: 'error',
      });
      continue;
    }
    ...
    for (const target of declaredSources) {
      ...
      if (!referenced) {
        ...
        findings.push({
          ...
          path: `${fid}::${relStr}`,
          message:
            `피처 ${fid} (status=done) 의 모듈 ${relStr} 가 src/ 어디에서도 참조되지 않음 — ` +
...
export function checkAnchorIntegration(
  harnessDir: string,
  spec: Record<string, unknown>,
  projectRoot: string | null = null,
): DriftFinding[] {
    ...
    for (const target of declaredSources) {
      ...
      if (!referenced) {
        findings.push({
          ...
          path: `${fid}::${relStr}`,
          message:
            `피처 ${fid} (status=done) 의 모듈 ${relStr} 가 integration_anchor (${anchorList}) ` +
...
export function checkSpecCoverage(
  harnessDir: string,
  _specYaml: Record<string, unknown> | null,
): DriftFinding[] {
  const findings: DriftFinding[] = [];
  ...
  if (isFile(harnessYamlPath)) {
    try {
      const cfg: unknown = yamlParse(readFileSync(harnessYamlPath, 'utf-8'));
      if (isPlainObject(cfg)) {
        const coverage = cfg['coverage'];
        if (isPlainObject(coverage)) {
          const override = (coverage as Record<string, unknown>)['threshold'];
  ...
  for (const name of entries) {
    ...
    const fid = (fp['feature_id'] as string | undefined) ?? name.replace(/\.yaml$/, '');
    ...
    for (const mismatch of mismatches) {
      ...
      if (ratio < threshold) {
        findings.push({
          ...
          message:
            `description claims ${descVal} ${metric} but AC accepts ${acVal} ` +
 ...
 */
export function checkAcceptanceTrace(
  harnessDir: string,
  ...
  const severity: Severity = strict ? 'error' : 'warn';
 ...
 */
function acTraceFailure(
  ac: unknown,
  fid: string,
  acN: string,
  testContents: ReadonlyMap<string, string>,
): string | null {
  ...
  return (
    `no test references both ${fid} and ${acN}. Add a test whose ` +
...
function collectTestFileContents(root: string): Map<string, string> {
...
const DEFAULT_SIGIL_DOCS = ['CLAUDE.md'] as const;
 ...
 */
export function checkContentDrift(
  harnessDir: string,
  projectRoot: string | null = null,
): DriftFinding[] {
  const findings: DriftFinding[] = [];
...
function resolveSigil(
  ...
  if (
    (declaredValue.length === 1 && /^[A-Z]$/.test(declaredValue)) ||
...
function resolveJsonField(content: string, field: string): string | null {
  try {
 ...
 */
function resolveTsEnumLike(content: string, name: string): string | null {
...
function resolveTsScalarConst(content: string, name: string): string | null {
  const esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(
...
export function runCheck(
  ...
  const report: CheckReport = {findings: [], checked: []};
  const harnessYaml = loadYamlFile(join(harnessDir, 'harness.yaml'));
 ...
 * ContentDrift.
 */
export function runBlockingCheck(
  harnessDir: string,
  projectRoot: string | null = null,
): CheckReport {
  const report: CheckReport = {findings: [], checked: []};
...
export function formatHuman(report: CheckReport): string {
  const lines: string[] = ['🔍 /harness:check', ''];
