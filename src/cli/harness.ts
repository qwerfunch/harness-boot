...
function buildProgram(): Command {
  ...
  program
    .command('export-spec')
    .description(
      'emit spec.yaml to stdout (read-only / CQS); --active-only ' +
    ...
    .action((options: Record<string, unknown>) => {
      ...
      const outputPath = typeof options['output'] === 'string' ? (options['output'] as string) : null;
      if (outputPath !== null) {
  ...
  program
    ...
    .action((options: Record<string, unknown>) => {
      ...
      if (options['json']) {
        ...
        printHuman(
          `token: recorded llm_call · ${tokensIn} in / ${tokensOut} out · ` +
      ...
      void import('../drive/checkpoint.js')
        .then(async ({loadCheckpoint, saveCheckpoint}) => {
          ...
          if (ck.phase === 'planning') {
            ...
            if (planOnly) {
              if (json) {
                ...
                printHuman(
                  `drive: Phase A done. Goal ${r.goalId} scaffolded with ${r.featureIds.length} features. ` +
          ...
          const r = runDriveLoop({
            ...
            maxRetries: options['maxRetries'] !== undefined ? Number(options['maxRetries']) : undefined,
            hardIterationLimit:
              options['hardStepLimit'] !== undefined ? Number(options['hardStepLimit']) : 100,
          });
          const summary: Record<string, unknown> = {
            proceed: r.proceed,
