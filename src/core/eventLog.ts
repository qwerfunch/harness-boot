 ...
 */
export type HarnessEvent = Record<string, unknown> & {ts?: unknown; type?: unknown};
...
function pythonStyleJsonStringify(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) {
      throw new TypeError(
        `eventLog: non-finite number cannot be serialized (${String(value)}).`,
      );
    }
 ...
 */
export function parseYyyymmFromTs(ts: unknown): string | null {
  if (typeof ts !== 'string') {
    return null;
  }
 ...
 */
function readJsonLines(path: string): HarnessEvent[] {
  let raw: string;
  try {
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) {
        out.push(parsed as HarnessEvent);
      }
    } catch {
 ...
 */
function rotatedPaths(harnessDir: string): string[] {
  let entries: string[];
  try {
 ...
 */
function eventSortKey(ev: HarnessEvent): [string, number] {
  const ts = ev.ts;
 ...
 */
export function* readEvents(harnessDir: string): IterableIterator<HarnessEvent> {
 ...
 */
export function rotate(harnessDir: string, options: RotateOptions = {}): RotateMoveMap {
  ...
  const keep: HarnessEvent[] = [];
  const buckets: Record<string, HarnessEvent[]> = {};
