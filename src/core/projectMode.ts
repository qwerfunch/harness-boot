 ...
 */
export function resolveMode(spec: unknown): Mode {
  if (spec === null || typeof spec !== 'object' || Array.isArray(spec)) {
    return DEFAULT_MODE;
  }
  const project = (spec as Record<string, unknown>)['project'];
  if (project === null || typeof project !== 'object' || Array.isArray(project)) {
    return DEFAULT_MODE;
  }
  const mode = (project as Record<string, unknown>)['mode'];
