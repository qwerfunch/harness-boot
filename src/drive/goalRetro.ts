...
import {State, type Feature} from '../core/state.js';
...
export function goalRetroPath(harnessDir: string, goalId: string): string {
 ...
 * no gate failed.
 */
function firstGateToFail(feature: Feature): string | null {
  const order = ['gate_0', 'gate_1', 'gate_2', 'gate_3', 'gate_4', 'gate_5'];
  if (!isPlainObject(feature.gates)) {
    return null;
  }
  for (const g of order) {
    const entry = (feature.gates as Record<string, unknown>)[g];
...
function countHaltsFromProgressLog(harnessDir: string): {total: number; perReason: Record<string, number>} {
 ...
 */
function featureRetroPreview(harnessDir: string, fid: string): string | null {
 ...
 * checkpoint.
 */
function composeRetro(
  goalSpec: GoalSpec,
  features: readonly Feature[],
  halts: {total: number; perReason: Record<string, number>},
  iteration: number,
  elapsedSec: number,
 ...
 */
export function generateGoalRetro(
  harnessDir: string,
  goalId: string,
  options: GenerateGoalRetroOptions = {},
): GenerateGoalRetroResult {
