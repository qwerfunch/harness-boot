...
const STANDARD_GATES = ['gate_0', 'gate_1', 'gate_2', 'gate_3', 'gate_5'] as const;
...
function featureIcon(status: FeatureStatus, emoji: boolean): string {
  if (!emoji) {
...
function countGatesPassed(feature: Feature): number {
 ...
 */
function countDeclaredEvidence(feature: Feature): number {
...
function elapsedMinutes(startedAt: string | null | undefined, now: Date): number | null {
...
function elapsedMinutesFromSec(elapsed_sec: number | undefined): number | null {
 ...
 */
function computePercentDone(features: readonly Feature[]): number {
...
function inflightGateLabel(feature: Feature): string | null {
  if (feature.status !== 'in_progress') {
    return null;
  }
  for (const g of STANDARD_GATES) {
    const entry = (feature.gates as Record<string, unknown> | undefined)?.[g];
 ...
 */
export function renderProgress(input: RenderInput, options: RenderOptions = {}): string {
  ...
  if (activeFeature !== null) {
    ...
    lines.push(
      `${arrow} now: ${activeFeature.id} / ${phase} · iteration ${iteration}${elapsedSegment}${haltSegment}`,
    );
  } else {
    const arrow = emoji ? '▶' : '>';
    const elapsedSegment = elapsedGoal !== null ? ` · ${elapsedGoal}m elapsed` : '';
 ...
 */
export function renderProgressJson(input: RenderInput, options: RenderOptions = {}): RenderJsonShape {
  const {goalSpec, goalRuntime, features} = input;
  const now = options.now ?? new Date();
...
function featureName(feature: Feature): string {
  const n = (feature as Feature & {name?: unknown}).name;
