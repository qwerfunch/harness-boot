 ...
 */
function readConfig(harnessDir: string): RealTestConfig | null {
  const path = join(harnessDir, 'harness.yaml');
  if (!existsSync(path)) {
    return null;
  }
  ...
  return {
    ...
    every_n_features:
      typeof everyN === 'number' && everyN > 0 ? Math.floor(everyN) : DEFAULT_EVERY_N_FEATURES,
  };
}
 ...
 */
function countDoneInGoal(state: State, goalId: string): number {
  const goal = state.getGoal(goalId);
  if (goal === null) {
    return 0;
  }
  const progress = goal.feature_progress as Record<string, string> | undefined;
 ...
 */
export function runRealTestIfDue(
  harnessDir: string,
  goalId: string | null,
  _completedFid: string,
): RealTestResult {
