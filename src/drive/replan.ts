...
import {State, type FeatureStatus} from '../core/state.js';

/** Keywords in the just-finished retro that trigger the file-drop manifest. */
const RETRO_PIVOT_KEYWORDS: readonly string[] = [
  'replan',
  'pivot',
  'rethink',
  'scope shift',
...
function emptyResult(): ReplanResult {
 ...
 */
function isReplanEnabled(harnessDir: string): boolean {
  const path = join(harnessDir, 'harness.yaml');
  if (!existsSync(path)) {
    return true;
  }
  try {
    const parsed = yamlParse(readFileSync(path, 'utf-8')) as Record<string, unknown> | null;
...
function alreadyDisabledOnce(harnessDir: string): boolean {
  const path = join(harnessDir, 'events.log');
  if (!existsSync(path)) {
    return false;
  }
 ...
 * manifest.
 */
function readRetro(harnessDir: string, completedFid: string): string | null {
 ...
 * (case-insensitive).
 */
function retroSuggestsReplan(retroText: string): boolean {
 ...
 */
function readSupersededIds(harnessDir: string): ReadonlySet<string> {
 ...
 */
export function replanAfterCompletion(
  harnessDir: string,
  completedFid: string,
  goalId: string | null,
): ReplanResult {
  ...
  if (deferred.length > 0) {
    ...
    if (newOrder.join(',') !== orderedIds.join(',')) {
      ...
      const newProgress: Record<string, FeatureStatus> = {};
 ...
 */
function renderManifest(goalId: string, completedFid: string, retroText: string): string {
  const lines: string[] = [];
  lines.push(`# Replan request — goal ${goalId}, after ${completedFid}`);
  lines.push('');
  lines.push(
    `Drive's replan hook detected pivot keywords in the retro for ${completedFid}. ` +
