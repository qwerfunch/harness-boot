...
import type {DependencyCategories, DirectoryPattern, Signals} from './signals.js';
...
function fact(key: string, value: string, source: string, label?: string): string {
  const visible = label ?? value;
  return `<!-- harness:fact key=${key} value=${value} source=${source} -->${visible}<!-- /harness:fact -->`;
}

function joinList(items: ReadonlyArray<string>, empty: string): string {
  return items.length === 0 ? empty : items.join(' · ');
}

function depBullets(
  deps: DependencyCategories,
): ReadonlyArray<readonly [keyof DependencyCategories, ReadonlyArray<string>]> {
  return [
...
function describeDirectoryPattern(pattern: DirectoryPattern): string {
  switch (pattern) {
    case 'src+tests':
...
function renderStack(s: Signals, lines: string[], counter: {n: number}): void {
  lines.push('## Stack', '');
  if (s.tech.runtime) {
  ...
  if (s.tech.min_version) {
    lines.push(
      `- Min version: ${fact('min_version', s.tech.min_version, s.manifests[0] ?? 'manifest')}`,
    );
    counter.n += 1;
  }
  if (s.tech.build) {
  ...
  for (const [bucket, members] of depBullets(s.dependencies)) {
    ...
    lines.push(
      `- ${capitalize(bucket)}: ${fact(`deps.${bucket}`, members.join(','), 'package.json/pyproject')}`,
    );
    counter.n += 1;
  }
  lines.push('');
...
function renderStyle(s: Signals, lines: string[], counter: {n: number}): void {
  lines.push('## Style', '');
...
function renderRules(s: Signals, lines: string[], counter: {n: number}): void {
  lines.push('## Rules', '');
...
function renderCommentsAndTests(lines: string[]): void {
  lines.push(
    '## Comments',
    '',
    ...
    '## Tests',
    '',
    '> _[pending: LLM hook stub — PR 3b fills sample-driven test-pattern detection]_',
    '',
  );
}

function renderImports(s: Signals, lines: string[], counter: {n: number}): void {
  lines.push('## Imports', '');
  if (s.styleConfigs.includes('tsconfig.json')) {
...
function renderDirectory(s: Signals, lines: string[], counter: {n: number}): void {
  lines.push('## Directory', '');
  lines.push(
    `- Pattern: ${fact('directory_pattern', s.directoryPattern, '.', describeDirectoryPattern(s.directoryPattern))}`,
  );
  counter.n += 1;
  if (s.i18n) {
 ...
 */
export function writeConventions(
  signals: Signals,
  targetPath: string,
): WriteConventionsResult {
  const lines: string[] = [];
  const counter = {n: 0};
  lines.push('# Conventions — auto-extracted (Layer-0 deterministic)', '');
  lines.push(
    'Generated by `harness init --scenario existing_code` (F-160).',
    'The Comments and Tests sections are filled by the slash command in PR 3b.',
    '',
  );
  ...
  lines.push(
    ...
    '## 사용자 보강 (자유 편집, reseed 시 보존)',
    '',
    '_reseed 가 이 영역을 건드리지 않습니다._',
