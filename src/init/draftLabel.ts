 ...
 */
export function computeContentHash(specYaml: string): string {
  const parsed = yamlParse(specYaml) as Record<string, unknown> | null;
  if (parsed && typeof parsed === 'object') {
    const meta = parsed['metadata'];
    if (meta && typeof meta === 'object' && !Array.isArray(meta)) {
      // Drop the content_hash field for stability.
      delete (meta as Record<string, unknown>)['content_hash'];
    }
  }
 ...
 */
export function stampDraftLabel(input: DraftLabelInput): DraftLabelResult {
  const parsed = (yamlParse(input.specYaml) ?? {}) as Record<string, unknown>;
  ...
  const hash =
    'sha256:' + createHash('sha256').update(withoutHash, 'utf8').digest('hex');
 ...
 */
export function specMatchesRecordedHash(specYaml: string): boolean | null {
  const parsed = yamlParse(specYaml) as Record<string, unknown> | null;
...
export function isDraft(specYaml: string): boolean {
  const parsed = yamlParse(specYaml) as Record<string, unknown> | null;
  const metadata = parsed?.['metadata'] as Record<string, unknown> | undefined;
