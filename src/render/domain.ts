...
function getPath(d: unknown, path: string, defaultValue: unknown = null): unknown {
  let cur: unknown = d;
 ...
 */
function multiline(text: string | null | undefined, prefix: string = ''): string {
  if (!text) {
    return '';
  }
...
export function render(
  ...
  if (stakeholders.length === 0) {
    ...
    for (const sh of stakeholders) {
      ...
      const role =
        asString(obj['role']) || asString(obj['id']) || '(unnamed)';
  ...
  if (risks.length === 0) {
    ...
    for (const r of risks) {
      ...
      lines.push(
        `**Likelihood × Impact**: ${likelihood} × ${impact} · status: ${status}${tagStr}`,
      );
      lines.push('');
      if (mitigation) {
