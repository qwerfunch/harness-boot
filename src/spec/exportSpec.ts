 ...
 */
export function exportSpec(harnessDir: string, options: ExportSpecOptions = {}): string {
  const specPath = join(harnessDir, 'spec.yaml');
  const specText = readFileSync(specPath, 'utf-8');
  const spec = yamlParse(specText) as Record<string, unknown> | null;
 ...
 */
function readShippedIds(harnessDir: string): ReadonlySet<string> {
  ...
  const state = yamlParse(raw) as Record<string, unknown> | null;
 ...
 */
function compactFeature(entry: unknown, shippedIds: ReadonlySet<string>): unknown {
  ...
  const compacted: Record<string, unknown> = {...record};
