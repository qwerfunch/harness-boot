...
const SUPPORTED: ReadonlySet<string> = new Set(['en', 'ko']);
const KOREAN_HINTS: ReadonlyArray<string> = ['ko', 'kor', 'KR'];
 ...
 */
export function resolveLang(spec: unknown = null): Lang {
  const envValue = process.env['HARNESS_LANG'];
  if (envValue && SUPPORTED.has(envValue)) {
    return envValue as Lang;
  }
