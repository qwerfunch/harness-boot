...
const EN: Readonly<Record<string, string>> = {
  ...
  iron_law_block:
    'cannot complete yet — {declared}/{required} evidence entries declared. Add more with --evidence.',
  ...
  tokens_line:
    '  tokens: {input} in / {output} out ({calls} calls)',
...
const KO: Readonly<Record<string, string>> = {
  ...
  iron_law_block:
    '아직 완료할 수 없어요 — 근거가 {declared}/{required} 개입니다. --evidence 로 더 추가하세요.',
...
const CATALOGS: Readonly<Record<Lang, Readonly<Record<string, string>>>> = {en: EN, ko: KO};

/** Every required catalog key — translation parity is asserted on this list. */
export const REQUIRED_KEYS: ReadonlyArray<string> = Object.keys(EN);
 ...
 *   placeholders.
 */
export function t(key: string, lang: Lang | string = 'en', fmt: Record<string, unknown> = {}): string {
  let catalog = CATALOGS[lang as Lang] ?? EN;
  if (!(key in catalog)) {
    if (!(key in EN)) {
